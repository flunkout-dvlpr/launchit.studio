...
Running it again later doesn't need a developer in the room. The `CLAUDE.md`'s "Running it again" section covers exactly that: how to re-run it day to day, which two files (`keywords.md` and `voice.md`) are the actual dials you're meant to turn, and what to do if Gmail access ever stops working, no code required for any of it.
